...
if $combineCard_flag; then
    ...
    for mass in "${masslist[@]}"; do
        ...
        combineCards.py ${datacardsDir}/${method}/2016/${scenario}_EBEB_2016/diphoton_combine_${mass}_${scenario}_EBEB_2016.txt \
            ...
            > diphoton_combine_${mass}_${scenario}_fullRun2_EBEB.txt;
    done
fi
...
if $combineLimit_flag; then
    ...
    # Calculate limits
    ./loopAllMassPoints.sh fullRun2 ${signal} ${coupling} ${method} ${datacardsDir} ${version} EBEB
    ...
    python ${plotLimit} -y fullRun2 -c ${coupling} -s ${signal} --unblind
fi
