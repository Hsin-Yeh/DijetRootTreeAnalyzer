...
time parallel --progress --jobs 10 'mass={1}; echo \"====================================================================\"; echo $mass; datacardfile=${mainpath}/${method}/${year}/diphoton_combine_${mass}_DiPhotons_${coupling}_${year}.txt); echo $datacardfile; echo \"combine -M AsymptoticLimits -s -1 --bypassFrequentistFit $datacardfile\"; combine -M AsymptoticLimits  -s -1 --bypassFrequentistFit $datacardfile > ${datacardfile}_results; mv higgsCombineTest.AsymptoticLimits.mH*.root combineJobs13TeV/${year}/${signal}/${coupling}/${combine_method}/All/.; obs=$(cat ${datacardfile}_results  | grep  \"Observed Limit:\" | awk \"{print $5}\"); expM2s=$(cat ${datacardfile}_results  | grep  \"Expected  2.5%:\" | awk \"{print $5}\"); expM1s=$(cat ${datacardfile}_results  | grep  \"Expected 16.0%:\" | awk \"{print $5}\"); exp=$(cat ${datacardfile}_results  | grep  \"Expected 50.0%:\" | awk \"{print $5}\"); expP1s=$(cat ${datacardfile}_results  | grep  \"Expected 84.0%:\" | awk \"{print $5}\"); expP2s=$(cat ${datacardfile}_results  | grep  \"Expected 97.5%:\" | awk \"{print $5}\"); echo $mass $obs $expM2s $expM1s $exp $expP1s $expP2s; echo $mass $obs $expM2s $expM1s $exp $expP1s $expP2s >> ${finalResults}' ::: $(seq ${massMin} ${massInterval} ${massMax})
