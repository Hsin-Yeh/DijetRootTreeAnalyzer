...
if __name__=='__main__':
  ...
  parser = OptionParser()
  parser.add_option('-l','--list',dest="li",type="string",default="none",help='filelist input')
  parser.add_option('-c','--cutFile',dest='cut',type="string",default="none",help='cut file in config/')
  ...
  FolderName = 'Condor/CondorSubmittion_%s_%04d%02d%02d_%02d%02d%02d' % (tag,current_time.year,current_time.month,current_time.day,current_time.hour,current_time.minute,current_time.second)
  ...
  for i in range(split):
    ...
    ExcuteFile.write('''
cd /afs/cern.ch/work/z/zhixing/private/CMSSW_9_4_0/src/CMSDIJET/DijetRootTreeAnalyzer2016/

eval `scramv1 runtime -sh`

'''+'\n\n')

    ExcuteFile.write('./main %s %s dijets/events %s %s'%('%s/FileList_%d.txt'%(FolderName,i+1),cutFile,'/tmp/'+OutputFile+'_'+str(i),'/tmp/'+OutputFile+'_'+str(i))+'\n\n')
