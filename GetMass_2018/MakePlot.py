...
csv_list = [0.0494, 0.1241, 0.2, 0.2770,0.35, 0.4184,0.45,0.5,0.55,0.6,0.65,0.7, 0.7264, 0.7527,0.8,0.85,0.9,0.95]  

L = [1530, 1607, 1687, 1770, 1856, 1945, 2037, 2132, 2231, 2332,2500,2700,3000,3300,3600,3900,4400,4800,5200,5600,6000,6500]


F=TFile('/afs/cern.ch/work/z/zhixing/private/CMSSW_7_4_14/src/CMSDIJET/DijetRootTreeAnalyzer/inputs/JetHT_run2018_red_cert_scan.root')
...
for i in ['CSV','DCSV','DeepJet']:
  for k in csv_list:
    ...
    gr[i]['le1'][k].Draw("APL")
    gr[i]['le1'][k].SetTitle('')
    gr[i]['le1'][k].GetXaxis().SetTitle('Dijet Mass(GeV)')
    gr[i]['le1'][k].GetYaxis().SetTitle('Tagging Rate')
...
g_le1btag_rate_Q = {}
leg = rt.TLegend(0.80, 0.65, 0.96, 0.74)
c1 = rt.TCanvas()
c1.SetGridx()
c1.SetGridy()
c1.cd()
g_le1btag_rate_Q['L'] = gr['DeepJet']['le1'][0.0494]
g_le1btag_rate_Q['M'] = gr['DeepJet']['le1'][0.2770]
g_le1btag_rate_Q['T'] = gr['DeepJet']['le1'][0.7264]
g_le1btag_rate_Q['L'].GetYaxis().SetRangeUser(0,1)
g_le1btag_rate_Q['L'].Draw('AL')
g_le1btag_rate_Q['L'].SetLineColor(rt.kRed)
leg.AddEntry(g_le1btag_rate_Q['L'],'Loose WP','L')
g_le1btag_rate_Q['M'].Draw('same')
g_le1btag_rate_Q['M'].SetLineColor(rt.kBlack)
leg.AddEntry(g_le1btag_rate_Q['M'],'Medium WP','L')
g_le1btag_rate_Q['T'].Draw('same')
g_le1btag_rate_Q['T'].SetLineColor(rt.kGreen)
leg.AddEntry(g_le1btag_rate_Q['T'],'Tight WP','L')
leg.Draw('same')
c1.Print('tagRate_LMT.pdf')
