...
from bTag_signalStudies_scanInter_DeepJet_2018 import *
...
def makeShape(mass, sample, model):
    ...
    for i in LIST: 
       ...
       myHisto2[i] = rt.TH1F("h2_"+model+"_Spring16_M"+str(mass)+"_Mjj_"+i,  "h2_"+model+"_Spring16_M"+str(mass)+"_Mjj_"+i, 70,0,9000 )
	...
	for i in LIST:
          ...
          myHisto2[i].Fill(Mjj[i])

    for i in LIST:
       myHisto[i].Scale(1./myHisto[i].GetEntries())
       myHisto2[i].Scale(1./myHisto2[i].GetEntries())
    return myHisto, myHisto2
...
#[0.1,0.15,0.2,0.3,0.35,0.45,0.5,0.5803,0.65,0.75,0.8,0.8838,0.9,0.9693]
    ...
    if (model == "qq"):
      ...
      flavor = 'bb'
      sampleNames = sampleNames_DMgq0p25['central']
    elif (model == 'DMgq0p2'):
      flavor = 'bb'
      sampleNames = sampleNames_DMgq0p2['central']
    elif (model == "gg"):
    ...
    for mass, sample in sorted(sampleNames.iteritems()):
       histo[mass],histo2[mass] = makeShape(mass,sample,model)
	...
	effFile = rt.TFile(options.eff+'/signalHistos_bb_'+str(int(i*1000))+'.root')  
      for Type in LIST:
	rootFile = rt.TFile(Folder+"/ResonanceShapes_"+model+"_"+flavor+"_13TeV_Spring16_"+str(int(i*1000))+'_'+Type+".root", 'recreate')
        for mass, sample in sorted(sampleNames.iteritems()):  
          if cate=='Non':
            g_eff = 'Non'
