        ...
        if (options.combmode == 'envelope'):
            for fitpdf in pdfIndexMap:
                ...
                if (fitpdf == 'envelope'):
                    ...
                    exec_me('combine -M MultiDimFit -m %s -d %s/diphoton_combine_%i_%s.txt %s %s %s --algo grid --cminDefaultMinimizerStrategy 0 --saveNLL -n %s_fixed_pdf_%d --X-rtd REMOVE_CONSTANT_ZERO_POINT=1 --X-rtd MINIMIZER_freezeDisassociatedParams'%(int(massPoint),options.outDir,int(massPoint),box,rRangeString,fixStringFit,freezeStringFit,box,pdfIndexMap[fitpdf]),options.dryRun)
