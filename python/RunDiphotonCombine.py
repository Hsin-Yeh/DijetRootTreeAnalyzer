...
if __name__ == '__main__':
    ...
    for massPoint in massIterable(options.mass):
        ...
        exec_me('combine -M AsymptoticLimits -s -1 %s/diphoton_combine_%i_%s_%s.txt  > %s/diphoton_combine_%i_%s_%s.txt_results'%(options.outDir,int(massPoint),box,options.year,options.outDir,int(massPoint),box,options.year))
