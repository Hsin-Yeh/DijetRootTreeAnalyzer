...
if __name__ == "__main__":
    ...
    for in_filename in args.in_filenames:
        ...
        h_vzdiff = ROOT.TH1D("h_vzdiff",basename,200, 0, 20)

        project(thetree, h_vzdiff, "abs(gen1vz-pvvz)", "")
        nTotal = h_vzdiff.Integral()
        n_dz10 = h_vzdiff.Integral(h_vzdiff.FindBin(0),h_vzdiff.FindBin(1))
        print 'dz < 1 = %d/%d (%.1f)'%(n_dz10, nTotal, (n_dz10/nTotal)*100)

        c1 = ROOT.TCanvas()
        c1.SetLogy()
        h_vzdiff.Draw()
        h_vzdiff.GetXaxis().SetTitle("|z_{gen}-z_{reco}| [cm]")
        leg = ROOT.TLegend(0.4,0.6,0.7,0.7)
        leg.SetHeader("dz < 1cm = %d/%d (%.1f%%)"%(n_dz10, nTotal, (n_dz10/nTotal)*100))
        leg.SetBorderSize(0)
        leg.SetTextSize(0.05)
        leg.Draw()
        c1.SaveAs("output/plots/vertexCheck/vertex_%s.png"%(basename))

        h_dR = ROOT.TH1D("h_dR",basename,200, 0, 20)
        project(thetree, h_dR, "deltaR", "")

        h_dR.Draw()
        h_dR.GetXaxis().SetTitle("|z_{gen}-z_{reco}| [cm]")
        leg = ROOT.TLegend(0.4,0.6,0.7,0.7)
        leg.SetHeader("dz < 1cm = %d/%d (%.1f%%)"%(n_dz10, nTotal, (n_dz10/nTotal)*100))
        leg.SetBorderSize(0)
        leg.SetTextSize(0.05)
