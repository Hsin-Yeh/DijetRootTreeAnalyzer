...
if __name__ == '__main__':
    ...
    if not options.multi:
        ...
        for f in args:
            ...
            # Find mean and fwhm of original distribution
            massMin=0.8*mass;
            massMax=1.2*mass;
            ...
            leg = rt.TLegend(0.65,0.45,0.95,0.6)
            leg.SetBorderSize(0)
            leg.SetLineColor(0)
            leg.SetFillColor(0)
            leg.SetFillStyle(0)
            ...
            pave_gof = rt.TPaveText(0.65,0.85,0.95,1.0,"NDC")

            h_mgg_varbins.SetLineColor(1)
            h_mgg_varbins.GetXaxis().SetRangeUser(massMin, massMax)
            h_mgg_varbins.GetXaxis().SetTitle("DiPhoton mass [GeV]")
            h_mgg_varbins.GetYaxis().SetTitle("Normalized yield/bin width")
            h_mgg_varbins.Scale(1/h_mgg_varbins.GetSumOfWeights())
            h_mgg_varbins.Draw("hist")
            ...
            rp = rt.TRatioPlot(hist_mass_list_rsg,h_mgg_varbins)
    ...
    if options.multi:
        ...
        for f in args:
            ...
            mass1 = mass
            tfileMin = rt.TFile.Open(f)
            tfileMin.cd()
            h_min = rt.TH1D('h_min','h_min',len(binBoundaries[options.coup])-1, array('d',binBoundaries[options.coup]) )
            thetreeMin=tfileMin.Get("HighMassDiphoton")
            massMin=0.8*mass1;
            massMax=1.2*mass1;
            h_mgg1 = rt.TH1D('h_mgg1','h_mgg1',1000,massMin,massMax)
            ...
            tfileMax.cd()
            h_max = rt.TH1D('h_max','h_max',len(binBoundaries[options.coup])-1, array('d',binBoundaries[options.coup]) )
            thetreeMax=tfileMax.Get("HighMassDiphoton")

            massMin=0.8*mass2;
            massMax=1.2*mass2;
            h_mgg2 = rt.TH1D('h_mgg2','h_mgg2',1000,massMin,massMax)
            ...
            canvas = rt.TCanvas("c_%s_%i_%i_%s_%s" %(title,mass1,mass2,options.cat,year),"c_%s_%i_%i_%s_%s" %(title,mass1,mass2,options.cat,year),50,50,W,H)
            canvas.SetFillColor(0)
            canvas.SetBorderMode(0)
            canvas.SetFrameFillStyle(0)
            canvas.SetFrameBorderMode(0)
            ...
            leg = rt.TLegend(0.2,0.7,0.9,0.9)
            leg.SetBorderSize(0)
            leg.SetLineColor(0)
            leg.SetFillColor(17)
            # leg.SetFillStyle(0)
            ...
            #Pave text
            pave_fit = rt.TPaveText(0.55,0.8,0.95,0.95,"NDC")
            ...
            i=0
            for mass in range(mass1+100, mass2, 100):
