...
if __name__ == '__main__':
    ...
    if 'DiPhotons' in box:
        if 'kMpl' in box: 
            signalDsName = '/afs/cern.ch/work/h/hsinyeh/public/diphoton-analysis/CMSSW_10_2_13/src/diphoton-analysis/DijetShapeInterpolator/%s/ResonanceShapes_InputShapes_RSGravitonToGammaGamma_%s_%s_%s.root'% (options.method, coupling, cat, options.year)
        else: 
            signalDsName = '/afs/cern.ch/work/h/hsinyeh/public/diphoton-analysis/CMSSW_10_2_13/src/diphoton-analysis/DijetShapeInterpolator/%s/ResonanceShapes_InputShapes_GluGluSpin0ToGammaGamma_W_%s_%s_%s.root'% (options.method, coupling, cat, options.year)
    ...
    for massPoint in massIterable(options.mass):        
        ...
        exec_me('combine -M FitDiagnostics --robustFit=1 %s/diphoton_combine_%i_%s_%s.txt -n %s_r-%.3f_%s_%s_%s_%s --toysFile %s -t %i %s %s %s --cminDefaultMinimizerStrategy=0 --saveWorkspace -v -1'%(
