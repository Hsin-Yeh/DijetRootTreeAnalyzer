...
if __name__ == '__main__':
    ...
    for f in args:
        ...
        h_mgg      = rt.TH1D('h_mgg','h_mgg',100,500,3000)
        h_qt       = rt.TH1D('h_qt','h_qt',100,0,1000)
        h_deltaR   = rt.TH1D('h_deltaR','h_deltaR',50,0,5)
        h_deltaEta = rt.TH1D('h_deltaEta','h_deltaEta',50,0,4)
        h_deltaPhi = rt.TH1D('h_deltaPhi','h_deltaPhi',50,0,4)
        h_ph1pt    = rt.TH1D('h_ph1pt','h_ph1pt',100,0,1000)
        h_ph1scEta = rt.TH1D('h_ph1scEta','h_ph1scEta',50,-2.,2.)
        h_ph2pt    = rt.TH1D('h_ph2pt'   ,'h_ph2pt',100,0,1000)
        h_ph2scEta = rt.TH1D('h_ph2scEta','h_ph2scEta',50,-2.,2.)

        thetree=tfileIn.Get("HighMassDiphoton")

        project(thetree,h_mgg      , "mgg"      , 'eventClass==%d && mgg > 1300 && mgg < 1400'%(cat))
        project(thetree,h_qt       , "qt"       , 'eventClass==%d && mgg > 1300 && mgg < 1400'%(cat))
        project(thetree,h_deltaR   , "deltaR"   , 'eventClass==%d && mgg > 1300 && mgg < 1400'%(cat))
        project(thetree,h_deltaEta , "deltaEta" , 'eventClass==%d && mgg > 1300 && mgg < 1400'%(cat))
        project(thetree,h_deltaPhi , "deltaPhi" , 'eventClass==%d && mgg > 1300 && mgg < 1400'%(cat))
        project(thetree,h_ph1pt    , "ph1pt"    , 'eventClass==%d && mgg > 1300 && mgg < 1400'%(cat))
        project(thetree,h_ph1scEta , "ph1scEta" , 'eventClass==%d && mgg > 1300 && mgg < 1400'%(cat))
        project(thetree,h_ph2pt    , "ph2pt"    , 'eventClass==%d && mgg > 1300 && mgg < 1400'%(cat))
        project(thetree,h_ph2scEta , "ph2scEta" , 'eventClass==%d && mgg > 1300 && mgg < 1400'%(cat))
