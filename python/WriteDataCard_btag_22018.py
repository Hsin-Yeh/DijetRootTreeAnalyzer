...
def writeDataCard(box,model,txtfileName,bkgs,paramNames,w,penalty,fixed,shapes=[],multi=False):
	...
	print bkgs
        ...
        if signals>1:
                ...
                if '2016' in box:
                        lumiErrs = [1.026 for sig in model.split('p')]
                elif '2017' in box:
                        lumiErrs = [1.026 for sig in model.split('p')]
                elif '2018' in box:
                        lumiErrs = [1.026 for sig in model.split('p')]
        ...
        lumiErrs.extend([1.026 for bkg in bkgs])
        divider = "------------------------------------------------------------\n"
...
def writeDataCardMC(box,model,txtfileName,bkgs,paramNames,w):
        ...
        if signals>1:
                ...
                processes = ["%s_%s"%(box,sig) for sig in model.split('p')]
                if '2016' in box:
                        lumiErrs = [1.026 for sig in model.split('p')]        #number of lumi's error 
                elif '2017' in box:
                        lumiErrs = [1.026 for sig in model.split('p')]        #number of lumi's error  
                elif '2018' in box:
                        lumiErrs = [1.026 for sig in model.split('p')]        #number of lumi's error   
...
if __name__ == '__main__':
    ...
    if options.noSignalSys:
        ...
        if options.btagUpFile is not None or options.btagDownFile is not None:
            shapes.append('btag_%s'%year)
            shapeFiles['btag_%sUp'%year] = options.btagUpFile
            shapeFiles['btag_%sDown'%year] = options.btagDownFile
    ...
    for shape in shapes:
		...
		print fUp, 'h_%s_%i'%(model,massPoint)
