...
def convertSideband(name,w,x):
    ...
    for ix in range(1,len(x)):
        iBinX+=1
        w.var('mgg').setVal((x[ix]+x[ix-1])/2.)
        inSideband = 0
        for fitname in names:
            inSideband += ( w.var('mgg').inRange(fitname) )
...
def addSignalHisto(h_bkg,data_obs_TGraph_,N_massBins_,massBins_,cat,lumi):
    ...
    # ci = rt.TColor.GetColor('#c85c17')
    # ci = rt.TColor.GetColor('#b16cab')
    # ci = rt.TColor.GetColor('#29b6f6')
    ci = rt.TColor.GetColor("#FF0000")
    # ci = rt.TColor.GetColor('#EE4B2B')
    # ci = rt.TColor.GetColor('#FF5B00')
    # ci = rt.TColor.GetColor('#FF0A01')
    # rt.gstyle.SetColorModelPS(1)
    sighist_1320.SetFillColorAlpha(ci,0.85)
    # sighist_1320.SetFillColorAlpha(rt.kRed,1)
    sighist_1320.SetLineColorAlpha(ci,0)
    h_residual_1320.SetFillColorAlpha(ci,0.85)
    # h_residual_1320.SetFillColorAlpha(rt.kRed,0.8)
    h_residual_1320.SetLineWidth(0)
    h_residual_1320.SetDirectory(0)

    # ci = rt.TColor.GetColor("#2cb6a5"); # Bird
    # ci = rt.TColor.GetColor("#869450")
    # ci = rt.TColor.GetColor("#4caf50")
    ci = rt.TColor.GetColor("#9933FF")
    sighist_2200.SetFillColorAlpha(rt.kViolet,0.85)
    # sighist_2200.SetFillColorAlpha(ci,0.8)
    ...
    # ci = rt.TColor.GetColor("#4caf50")
    ci = rt.TColor.GetColor("#9933FF")
    # h_residual_2200.SetFillColorAlpha(ci,0.8)
    h_residual_2200.SetFillColorAlpha(rt.kViolet,0.85)
    ...
    for bin in range (0,N_massBins_):
        ...
        h_residual_1320.SetBinContent(bin+1,residual_1320)
        h_residual_1320.SetBinError(bin+1,err_residual_1320)
        h_residual_2200.SetBinContent(bin+1,residual_2200)
        h_residual_2200.SetBinError(bin+1,err_residual_2200)
...
if __name__ == '__main__':
    ...
    for i in range(1,nBins+1):
        ...
        plotRegions = plotRegion.split(',')
        checkInRegions = [myRebinnedDensityTH1.GetXaxis().GetBinCenter(i)>w.var('mgg').getMin(reg) and myRebinnedDensityTH1.GetXaxis().GetBinCenter(i)<w.var('mgg').getMax(reg) for reg in plotRegions]
        if not any(checkInRegions):
    ...
    if options.year=="fullRun2": chiText.SetTextSize(0.048)
    ...
    h_fit_residual_vs_mass.SetFillColor(rt.kBlack)
    ...
    legres_data.SetTextSize(0.078)
    legres_data.SetFillColor(rt.kWhite)
    legres_data.SetFillStyle(0)
    ...
    if 'DiPhoton' in box:
        ...
        h_fit_residual_vs_mass.GetXaxis().SetTitle('m_{#gamma#gamma} (TeV)')
        h_fit_residual_vs_mass.GetXaxis().SetLabelOffset(1000)
        h_fit_residual_vs_mass.GetXaxis().SetTickSize(0.06)
        #h_fit_residual_vs_mass.GetXaxis().SetNoExponent()
        #h_fit_residual_vs_mass.GetXaxis().SetMoreLogLabels()
        xLab = rt.TLatex()
        xLab.SetTextAlign(22)
    ...
    if not options.linearX:
        if (options.PAS):
            c.Print(options.outDir+"/fit_mgg_%s_%s_%s_PAS.pdf"%(fitRegion.replace(',','_'),box,options.year))
        else:
    ...
    rootFile.cd()
    c.Write()
    backgrounds["dijet"].SetTitle("Dijet")
    backgrounds["expow1"].SetTitle("Expow1")
    backgrounds["invpow1"].SetTitle("Invpow1")
    backgrounds["invpowlin1"].SetTitle("Invpowlin1")
    backgrounds["dijet"].SetName("Dijet")
    backgrounds["expow1"].SetName("Expow1")
    backgrounds["invpow1"].SetName("Invpow1")
    backgrounds["invpowlin1"].SetName("Invpowlin1")
