...
if __name__ == '__main__':
    ...
    EBText=rt.TLatex(0.39,0.83, "%s"%(options.cat));
    EBText.SetNDC(1);
    ...
    legres = rt.TLegend(0.63,0.3,0.9,0.52)
    legres.SetTextFont(42)
    legres.SetTextSize(0.055)
    legres.SetFillColor(rt.kWhite)
    legres.SetFillStyle(0)
    legres.SetLineWidth(0)
    legres.SetLineColor(rt.kWhite)
    legres.AddEntry(h_fit_residual_vs_mass,"(Data-Fit)/Unc.","f")
    legres.AddEntry(h_residual_1320,"Signal/Unc. (M_{G}=1.3 TeV)","f")
    legres.AddEntry(h_residual_2200,"Signal/Unc. (M_{G}=2.2 TeV)","f")
    legres.Draw()
