...
def getNormalization(SignalNormFile):
    norm = pd.read_csv(SignalNormFile, sep=" ", header=None)
...
def initializeWorkspace(w,cfg,box,scaleFactor=1.,penalty=False,multi=False,x=None,emptyHist1D=None):
    ...
    if multi:
        #print(pdf_index)
        # print(box)
        paramNames.append('pdf_index_%s_%s'%(options.box.split('_')[2],options.box.split('_')[3])) # DiPhotons_kMpl001_EBEB_2016
        bkgs = ['multi']
...
def writeDataCard(box,model,txtfileName,bkgs,paramNames,w,penalty,fixed,year,shapes=[],multi=False):
        ...
        # now nuisances
        datacard+=lumiString+pdfsString#+effString
        for shape in shapes:
...
if __name__ == '__main__':
    ...
    norm = getNormalization(options.SignalNormFile)
    ...
    for name in names:
        ...
        if isinstance(d, rt.TH1):
            ...
            if name=='h_%s_%i'%(model,massPoint):
                ...
                print(sigDataHist_mgg.sumEntries())
    ...
    if options.inputFitFile is not None:
        ...
        if wIn.obj("fitresult_extDijetPdf_data_obs") != None:
            ...
            frIn = wIn.obj("nll_extDijetPdf_%s_data_obs"% box.split("DiPhotons_")[1])  #FIXME I add "_dijet to the original name"
